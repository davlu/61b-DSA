...
public class AStarSolver<Vertex> implements ShortestPathsSolver<Vertex> {
    ...
    public AStarSolver(AStarGraph<Vertex> input, Vertex start, Vertex end, double timeout){
        ...
        while(fringe.size()!=0 && fringe.getSmallest() != end && result != 1){
            ...
            numDequeues+=1;
            for(WeightedEdge<Vertex> edge : input.neighbors(currentVertex)){ //looking at all weighted edge from curr V
    ...
    private void relax(AStarGraph<Vertex> input, WeightedEdge<Vertex> edge, Vertex currentVertex, Vertex end){
        Vertex next = edge.to(); //destination vertex from source vertex
        Vertex current = edge.from();
        double edgeWeight = edge.weight();
        double heuristic = input.estimatedDistanceToGoal(next, end); //heuristic for nextVertex
        double currentPlusEdge = distTo.get(current)+edgeWeight;
        if(!distTo.containsKey(next)){ //if distTo doesn't contain nextVertex..
            distTo.put(next, edge.weight()+distTo.get(currentVertex)); //add distance from source + weight
        }
